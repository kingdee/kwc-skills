...
Table 组件支持通过 `:onRow`、`:onHeaderRow`、`onCell`、`onHeaderCell` 回调函数自定义行和单元格的属性，包括 className、style、事件监听器等。在 Vue 中，回调函数直接使用箭头函数，可访问 `ref` 响应式变量，无需 `const self = this` 闭包。
...
function handleRow(record, index) {
  return {
    ...
    onClick: () => {
      clickedRow.value = `${record.name} (ID: ${record.id})`;
    }
  };
}
</script>
...
<template>
  <sl-table
    ...
    :columns.prop="columns"
    :dataSource.prop="dataSource"
    :onHeaderRow.prop="handleHeaderRow"
  ></sl-table>
</template>
...
function handleHeaderRow(columns, index) {
  return {
...
组合使用所有属性定制功能，通过 `ref` 响应式变量在回调中追踪悬停状态。
...
function handleRow(record, index) {
  const isHovered = hoveredRow.value === record.id;
  return {
    style: isHovered ? 'box-shadow: 0 2px 8px rgba(0,0,0,0.15);' : '',
    onMouseEnter: () => { hoveredRow.value = record.id; },
...
2. **Vue 无需 self 闭包**：Vue 中回调函数使用箭头函数，可直接访问 `ref` 变量（如 `hoveredRow.value`），不需要 LWC 中的 `const self = this`
