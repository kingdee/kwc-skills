...
Table 组件支持列排序功能，通过在列配置中设置 `sorter` 函数开启。支持升序（asc）和降序（desc）两种排序方向，可通过 `defaultSortOrder` 设置默认排序方向，通过监听 `@sl-change` 事件获取排序状态变化。
...
通过 `@sl-change` 监听排序状态变化，在模板中直接绑定，无需在 JS 中手动添加事件监听。
...
const sortColumn = ref('无');
const sortDirection = ref('无');
...
function handleChange(event) {
  ...
  if (changeType === 'sorter') {
    if (sorting && sorting.length > 0) {
      ...
      sortColumn.value = sort.id;
      sortDirection.value = sort.desc ? '降序' : '升序';
    } else {
      sortColumn.value = '无';
      sortDirection.value = '无';
    }
  }
}
</script>
...
6. **Vue 事件绑定**：在模板中使用 `@sl-change` 绑定，通过 `event.detail` 获取排序状态，无需 JS 手动绑定
