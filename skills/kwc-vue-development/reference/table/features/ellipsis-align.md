...
4. **className 全局样式**：`className` 会应用到该列的所有单元格，建议将对应样式写在全局 CSS 中（不使用 `scoped`），或根据组件的样式穿透方式处理
5. **Vue 中 bordered**：直接写 `bordered` 属性（不需要赋值），等同于 `:bordered="true"`
