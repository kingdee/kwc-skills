...
5. 每次重新上传页面元数据前，手动将 `version` 加 `1`。
...
3. 若仅是内部组件实现变更，但该组件仍有 `.js-meta.kwc`，它依然会随 deploy 上传
...
在完成组件和页面元数据后，默认要把交付链路推进到部署，而不是停在本地代码层面：

1. 运行 `kd project deploy` 部署整个项目。
2. 若用户只想部署某个组件，使用 `kd project deploy -d <path> -e <env-name>`。
3. 运行 `kd debug` 进入调试。
4. 若需要指定环境或表单，使用 `kd debug -e <env-name>` 或 `kd debug -f <formid>`。
