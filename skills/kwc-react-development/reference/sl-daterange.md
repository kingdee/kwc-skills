...
import SlDaterange from '@kdcloudjs/shoelace/dist/react/daterange/index.js';

export default () => <SlDaterange style={{ width: '400px' }} />;
```
...
export default () => <SlDaterange style={{ width: '400px' }} value={[new Date(2025, 0, 1), new Date(2025, 5, 30)]} />;
```
...
export default () => <SlDaterange style={{ width: '400px' }} label="选择日期范围" />;
```
...
export default () => <SlDaterange style={{ width: '400px' }} label="入住退房" help-text="请选择入住和退房日期" />;
```
...
export default () => <SlDaterange style={{ width: '400px' }} placeholder={['Check-in', 'Check-out']} />;
```
...
export default () => (
  <>
    <div className="daterange-group">
      <SlDaterange value={[new Date(2025, 0, 1), new Date(2025, 5, 30)]} label="默认可清除" />
      <SlDaterange value={[new Date(2025, 0, 1), new Date(2025, 5, 30)]} label="不可清除" clearable={false} />
    </div>
    <style>{css}</style>
  </>
);
...
export default () => <SlDaterange style={{ width: '500px' }} label="日期时间范围" type="datetime" />;
```
...
export default () => <SlDaterange style={{ width: '400px' }} label="填充样式" filled />;
```
...
export default () => <SlDaterange style={{ width: '400px' }} label="胶囊样式" pill />;
```
...
export default () => (
  <>
    <div className="daterange-group">
      <SlDaterange label="Small" size="small" />
      <SlDaterange label="Medium" size="medium" />
      <SlDaterange label="Large" size="large" />
    </div>
    <style>{css}</style>
  </>
);
...
export default () => (
  <SlDaterange style={{ width: '400px' }} value={[new Date(2025, 0, 1), new Date(2025, 5, 30)]} disabled />
);
```
...
export default () => (
  <SlDaterange style={{ width: '400px' }} value={[new Date(2025, 0, 1), new Date(2025, 5, 30)]} readonly />
);
```
...
export default () => <SlDaterange style={{ width: '400px' }} label="限制范围" min="2025-01-01" max="2025-12-31" />;
```

## 监听日期变化事件

`SlDaterange` 的 React 包装器已映射 `sl-*` 事件，可直接在 JSX 中使用 `onSlChange`、`onSlInput` 等属性。
...
import SlDaterange from '@kdcloudjs/shoelace/dist/react/daterange/index.js';
import type SlDaterangeElement from '@kdcloudjs/shoelace/dist/components/daterange/daterange.js';
...
export default () => {
  ...
  const handleChange = useCallback((event: CustomEvent) => {
    const target = event.target as SlDaterangeElement;
...
export default () => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', width: '400px' }}>
      ...
      <SlDaterange ref={daterangeRef} label="日期范围" />
    </div>
  );
};
```
...
export default () => {
  const daterangeRef = useRef<SlDaterangeElement>(null);
...
export default () => (
  <SlDaterange style={{ width: '400px' }} label="自定义后缀">
...
`SlDaterange` 的 React 包装器已映射以下事件为 JSX 属性，可以直接在 JSX 中使用。
...
```jsx
// React JSX 设置（推荐）
<SlDaterange value={[new Date(2025, 0, 1), new Date(2025, 5, 30)]} />
<SlDaterange value={[null, null]} />  // 清空
...
```jsx
// React JSX 设置
<SlDaterange type="datetime" value={[new Date(2025, 0, 1, 8, 0, 0), new Date(2025, 5, 30, 18, 0, 0)]} />;
...
A: 使用 `min` 和 `max` 属性：`<SlDaterange min="2025-01-01" max="2025-12-31" />`。超出范围的日期会被自动禁用。
