...
通过 `className` 为特定列添加自定义样式。注意需要在全局 CSS 中定义对应的类名样式。
...
// 全局 CSS（在项目入口或全局样式文件中定义）:
// .status-column { font-weight: bold; }
// .score-column { color: var(--sl-color-primary-600); }
...
// 全局 CSS:
// .id-column { background-color: var(--sl-color-neutral-50); }
// .budget-column { font-family: 'Courier New', monospace; color: var(--sl-color-success-600); }
...
4. **className 全局样式**：`className` 会应用到该列的所有单元格，需在全局 CSS 文件中定义对应样式，不能使用 CSS Modules 中的局部样式
5. **React 中 bordered**：在 JSX 中直接写 `bordered` 属性等同于 `bordered={true}`
