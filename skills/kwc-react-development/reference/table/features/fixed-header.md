...
export default () => {
  ...
  const updateTableHeight = useCallback(() => {
    setTableHeight(Math.max(200, window.innerHeight - 200));
  }, []);
  ...
  const style = { padding: "var(--sl-spacing-medium)" };
  return (
    <div style={style}>
      <SlTable rowKey="id" columns={columns} dataSource={dataSource} tableScroll={tableScroll} />
    </div>
...
5. **React 动态高度**：使用 `useMemo` 缓存 `tableScroll` 对象，结合 `useState` 实现响应式高度
6. **React 属性名**：React 中使用 `tableScroll` 属性（camelCase），不是 `table-scroll`
