...
通过 `ref` 动态更新分段选择器的选项。当选项变化时，如果当前选中值不在新选项中，会自动选中第一个非禁用选项。
...
export default () => {
  ...
  const handleLoadMore = () => {
    if (segRef.current) {
      segRef.current.options = ['Daily', 'Weekly', 'Monthly', 'Quarterly', 'Yearly'];
    }
  };

  const handleReset = () => {
    if (segRef.current) {
      segRef.current.options = ['Daily', 'Weekly', 'Monthly'];
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      <div style={{ display: 'flex', gap: '8px' }}>
        <SlButton size="small" onClick={handleLoadMore}>
          加载更多选项
        </SlButton>
      ...
      <SlSegmented ref={segRef} options={['Daily', 'Weekly', 'Monthly']} />
    </div>
  );
};
```
