...
export default class BasicTable extends KingdeeElement {
    columns = [
...
export default class BorderedTable extends KingdeeElement {
    columns = [
...
设置 `show-header="false"` 隐藏表头，适用于简洁展示场景。
...
export default class NoHeaderTable extends KingdeeElement {
    columns = [
...
export default class CustomRowKeyTable extends KingdeeElement {
    columns = [
...
1. **rowKey 必须唯一**：`row-key` 指定的字段在数据中必须唯一，否则会导致行选择、展开等功能异常
