...
export default class LinkedDatepickers extends KingdeeElement {
    ...
    renderedCallback() {
        if (this._eventsBound) return;
        this._eventsBound = true;
        const join_datepicker = this.template.querySelector('.join-datepicker');
        if (join_datepicker) {
            join_datepicker.addEventListener('sl-change', this.handleJoinDateChange.bind(this));
        }
        const probation_datepicker = this.template.querySelector('.probation-datepicker');
        if (probation_datepicker) {
            probation_datepicker.addEventListener('sl-change', this.handleProbationChange.bind(this));
        }
        const regular_datepicker = this.template.querySelector('.regular-datepicker');
        if (regular_datepicker) {
            regular_datepicker.addEventListener('sl-change', this.handleRegularChange.bind(this));
        }
    }
